...
\]
When restricted to a one-dimensional subspace, the multivariate Stieltjes transform coincides with the univariate Stieltjes transform of the Radon transform of $f$ in the direction of the subspace. That is, if $y = z\omega$ for any $\omega \in S^{n-1}$ and $z \in \RR$,
\[
...
% [GRT of polynomials]
Unlike the RT, the GRT is well defined on the vector space of the polynomials on $\RR^n$, raising the question, addressed in chapter \ref{chap:GRTpoly}, of how exactly the GRT acts on polynomials.

Exploratory computation in the $2$-dimensional case suggests the relation
\[
  GR_{H_k H_\ell}(\omega, p) = H_{k + \ell}(p) \omega_1^k \omega_2^\ell, \quad k, \ell \in \NN_0,
\]
where $\{H_k\}_{k \in \NN_0}$ are the Hermite polynomials and $\omega = (\omega_1, \omega_2) \in S^1$, from which we form and prove the following generalization in chapter \ref{chap:GRTpoly}:
% Some early computation indicate that there is a simple relationship between the Hermite polynomials and the GRT.\@ For example in $\RR^2$, the tensor product multivariate Hermite polynomials given by 
% \[
...
We extend this result further to a generalized ``$d$-plane'' Gaussian Radon Transform in the same chapter.
