...
\item RT/GRT.\@ Notation for Gaussian measure on hyperplanes is not good. The symbols $w$ and $\omega$ are too similar. Also is it $w_{n-1}(x - p\omega) = \exp(-\|x - p\omega\|_{\RR^{n-1}}^2)$ or $w_{n-1}(x - p\omega) = \exp(-\|x - p\omega\|_{\RR^n}^2)$? Equivalent? Notation is confusing.
