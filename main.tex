...
Let $\mu$ be a positive Borel measure with infinite support and finite moments $c_k = \int_{-\infty}^\infty p^k d\mu$. 
...
Let us consider the domain of definition for $g(z)$. The denominator $1 + zp$ vanishes whenever $z = -\frac1p$. Since $p \in \RR$ it seems that $g(z)$ is at least defined when $\im z \neq 0$. In fact:
...
The function $g(z)$ is a sort of generating function encoding the moment sequence $(c_k)_{k \in \NN_0}$ of the measure $\mu$. In particular it admits a formal assymptotic expansion $\sum_{n=0}^\infty c_n(-z)^n$, which we will call the Markov (Hamburger resp.) series.
\begin{proposition}
For all $n \in \NN_0$
\begin{equation}
    g(z) = c_0 - c_1z + c_2z^2 - \cdots + c_n(-z)^n + z^n h_n(z) \label{assexp}
\end{equation}
where $\lim_{z \rightarrow 0} h_n(z) = 0$ non-tangentially.
\end{proposition}
