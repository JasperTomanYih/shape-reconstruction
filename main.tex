...
\item Intro. Discuss in the role of the Radon transform in proofs surrounding the method, as well as its extension to the Gaussian transform.
\item RT/GRT.\@ Notation for Gaussian measure on hyperplanes is not good. The symbols $w$ and $\omega$ are too similar. Also is it $w_{n-1}(x - p\omega) = \exp(-\|x - p\omega\|_{\RR^{n-1}}^2)$ or $w_{n-1}(x - p\omega) = \exp(-\|x - p\omega\|_{\RR^n}^2)$?
...
% \[
%     g(z) = \int_{\RR^n} \frac{f(x)}{1 + z\langle x, \omega \rangle} ~dx
% \]
\begin{align*}
