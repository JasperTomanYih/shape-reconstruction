...
\begin{definition}
  ...
  \[
    c^G_\alpha = \int_{\RR^n} f(x)x^\alpha \gamma(x) ~dx
  \]
  for $\alpha \in \NN_0^n$.
\end{definition}
...
\begin{myexample}
  ...
  \[
    c^G_k = \int_{-\infty}^\infty p^k \gamma(p) ~dp = 0
  \]
...
\begin{myexample}
  ...
  \end{align*}
  where $\{c^G_k\}_{k \in \NN}$ are the univariate moments of $\gamma(p)$.
...
The Hermite polynomials form a complete orthogonal basis for $L^2(\RR, w)$, the space of function $f : \RR \to \RR$ for which $\int_{-\infty}^\infty f(p)^2 \gamma(p)~dp < \infty$. This is a Hilbert space with inner product $\langle f, g \rangle = \int_{-\infty}^\infty |f(p)g(p)| \gamma(p)~dp$.
...
\[
  H_\alpha(x) 
    = (-1)^{|\alpha|}\frac{\partial_{\alpha}\gamma_n(x)}{w_n(x)}
\]
where $\partial_\alpha = \frac{\partial^{\alpha_1 + \cdots + \alpha_n}}{\partial x_1^{\alpha_1} \cdots \partial x_n^{\alpha_n}}$. Just like the classical case, this formula can be used to define $H_\alpha$ as multivariate Taylor coefficients for the generating function:
\[
