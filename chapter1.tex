...
\begin{lemma}
  Let $f(x) = x^\alpha$ for some multiindex $\alpha \in \NN_0^n$ of degree $|\alpha| = k$. Then the rotation $f(Rx)$ has the expansion
...
\end{remark}
\begin{myexample}
  For a simple example take the monomial $f(x) = x_1^2x_2$. Here $n = 2$, $\alpha = (2, 1)$ and $k = 3$. The partition of $\{1, 2\}^3$ over multi-indeces $|(\beta_1, \beta_2)| = 3$ is given by
  \begin{align*}
    I(3,0) &= \{(1, 1, 1)\} \\
...
\begin{proposition}
  ...
  \]
  where $c^G_\beta = \int_{\RR^d} x^\beta w_d(x) ~dx$ are the Gaussian moments on $\RR^d$.
\end{proposition}
...
\begin{proposition}
  The GRT of a multivariate Hermite polynomial $H_\alpha$ on a linear subspace $\Lambda_0$ is given by
  \[
    GR_{H_\alpha}(\Lambda_0) = \sum_{\substack{\beta \in \NN_0^n \\ |\beta| = k}} 
  \]
\end{proposition}
