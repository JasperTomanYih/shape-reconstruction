...
\section{Basic Notation}\label{sec:notation}
...
% Throughout this work we use standard notation for some common function spaces:

% \begin{itemize}
  % \item 
  For $1 \leq p < \infty$, $L^p = L^p(\RR^n)$ is the space of Lebesgue measurable functions $f : \RR^n \to \RR$ satisfying
  \[
    \int_{\RR^n} |f(x)|^p ~dx < \infty.
  \]
  As usual these functions are only defined modulo differences on sets of Lebesgue measure zero. For the Gaussian weight $\gamma_n$ we similarly define $L^p(\gamma_n)$ for functions satisfying
  \[
    \int_{\RR^n} |f(x)|^p \gamma_n(x) ~dx < \infty
  \]
  modulo Lebesgue measure zero sets.

  % \item For $m \in \NN$ we denote by $C^m = C^m(\RR^n)$ the space of $m$-times continuously differentiable functions on $\RR^n$, and by $C^\infty = C^\infty(\RR^n)$ the space of smooth functions. The subspaces of bounded functions, those with compact support, and those vanishing at infinity are denoted by $C_b^m$, $C_c^m$, and $C_0^m$ respectively, where $m \in \NN \cup \{\infty\}$.
  
  % \item The set of real polynomials on $\RR^n$ of degree at most $k$ is denoted $\mathcal{P}_k = \mathcal{P}_k(\RR^n)$ and the set polynomials of arbitrary degreee $\mathcal{P} = \mathcal{P}(\RR^n)$.
% \end{itemize}

\section{Classical and multivariate moment problems} \label{sec:momentproblem}
...
\begin{enumerate}
  ...
  \item When $\mu$ is the Lebesgue measure on a set $A \subset \RR$ of finite measure, moments can be defined by
  \[
    c_k = \int_A x^k dx.
  \]
\end{enumerate}
...
% For our purposes we will assume existence. 
...
In the classical cases (Markov, Stieltjes, Hamburger) these questions have been more or less resolved. Precise conditions for solvable and determinate moment problems have been found, and the nature of solution sets to indeterminate moment problems are well understood. A good reference for the theory of classical moment problems can be found in \cite{A2021}.
...
We now define multivariate moment problems. Let $\mu$ be a Borel measure, now defined on $\RR^n$. For $\alpha \in \NN_0^n$. Define the $\alpha$th multivariate moment of $f$ as 
\[
...
\[
  \frac{\partial}{\partial x_i} \gamma_n(x) = -x_i \gamma_n(x), \qquad i = 1, \ldots, n.
\]
Notably, the exponential decay of $\gamma_n(x)$ at infinity means that the Gaussian weight dominates polynomial growth in the sense that 
\[
  \int_{\RR^n} P(x) \gamma_n(x) ~dx < \infty,
\]
for all polynomials $P : \RR^n \to \RR$.

% Third: The Gaussian function dominates polynomials in the sense that
% \[
%   \lim_{p \rightarrow -\infty} P(p)\gamma(p) = 0 = \lim_{p \rightarrow \infty} P(p)\gamma(p)
% \]
...
\begin{definition}
  The \textit{Gaussian moments} of a function $f: \RR^n \rightarrow [0, \infty)$ are,
  \[
    c^\gamma_\alpha = \int_{\RR^n} f(x)x^\alpha \gamma(x) ~dx
  \]
  for $\alpha \in \NN_0^n$, provided they exist.
\end{definition}

\begin{remark}
  The Gaussian moments asocciated with a non-negative function $f$ can be understood simply as the moments of the Gaussian-weighted function $f \gamma$. Alternatively we might think of them as a type of generalized moment. For motivation notice that if $X$ is a standard normal random variable then for $\alpha \in \NN_0^n$, the expected value of $f(X) X^\alpha$ where is precisely $c_\alpha^\gamma$.
\end{remark}


Next we prove a sufficient condition for the existence and determinacy of Gaussian moments by way of the well known \emph{Cramer's condition}.

\begin{proposition}[Cramer's condition]

  Let $\mu$ be a Radon measure on $\RR$. If there exists $\epsilon > 0$ such that
  \[
    \int_{-\infty}^\infty e^{\epsilon |p|} ~d\mu(p) < \infty
  \]
  then $\mu$ has finite and determinate moments.
  
\end{proposition}

\begin{theorem}

  Let $f : \RR \to [0, \infty)$ be a non-negative measurable function. If 
  \[
    \int_{-\infty}^\infty f(p)^2 \gamma(p) dp < \infty,
  \]
  then the Gaussian moments
  \[
    c^\gamma_k = \int_{-\infty}^\infty f(p) p^k \gamma(p) dp
  \]
  exist (are finite) and determinate.
...
\begin{proof}
  ...
  \begin{equation} \label{eq:expGaussConvergence}
    \int_{-\infty}^\infty e^{\epsilon |p|} \gamma(p) dp < \infty
  \end{equation}
  for any $\epsilon > 0$. This is somewhat stronger than we need, but suffices for Cramer's condition. First note that by completing the square

  \begin{align*}
    e^{\epsilon |p|} \gamma(p) = (2\pi)^{-\frac12} e^{\epsilon |p| - \frac{p^2}2} =  (2\pi)^{-\frac12} e^{-\frac{\epsilon^2}2} e^{-\frac12 (\epsilon - |p|)^2} = e^{-\frac{\epsilon^2}2} \gamma(\epsilon - |p|).
  \end{align*}

  Thus we can rewrite

  \begin{align*}
    \int_{-\infty}^\infty e^{\epsilon |p|} \gamma(p) dp 
    % &= (2\pi)^{-\frac 12} \int_{-\infty}^\infty e^{\epsilon |p| - \frac{p^2}2} dp \\
    &= e^{-\frac{\epsilon^2}2} \int_{-\infty}^\infty \gamma(\epsilon - |p|) ~dp.
  \end{align*}

  Now the right hand integral can be compared to the Gaussian integral
  
  \begin{align*}
    \int_{-\infty}^\infty \gamma(\epsilon - |p|) ~dp
    = 2 \int_0^\infty \gamma(\epsilon - p) ~dp < 2 \int_{-\infty}^\infty \gamma(\epsilon - p) = 2,
  \end{align*}
  
  so that \ref{eq:expGaussConvergence} holds for arbitrary $\epsilon > 0$. In particular we will use the fact that
  \[
    \int_{-\infty}^\infty (e^{|p|})^2 \gamma(p) dp = \int_{-\infty}^\infty e^{2|p|} \gamma(p) dp < \infty.
  \]

  Let $f(p)$ satisfy the conditions of the theorem. Then by the Cauchy-Shwarz inequality we have
  \[
    \int_{-\infty}^\infty e^{|p|} f(p) \gamma(p) dp \leq \int_{-\infty}^\infty (e^{|p|})^2 \gamma(p) dp \int_{-\infty}^\infty f(x)^2 \gamma(p) dp < \infty.
  \]
  Thus by Cramer's condition $f(p) \gamma(p)$ has finite and determinate moments.
\end{proof}


\begin{myexample}
  Let $f: \RR \rightarrow \RR$ be the constant function $f(p)=1$. The Gaussian moments of $f$, i.e.\ the moments of $\gamma(p)$, are
  \[
    c^\gamma_k 
    = \begin{cases}
        (k-1)!!, & k~\text{even} \\
  ...
  \[
    c^\gamma_k = \int_{-\infty}^\infty p^k \gamma(p) ~dp = 0
  \]
  ...
  \begin{align*}
    c^\gamma_{2\ell}
    &= \int_{-\infty}^\infty p^{2\ell} \gamma(p) ~dp \\
    &= -\int_{-\infty}^\infty p^{2\ell-1} \left(-p \gamma(p)\right) ~dp \\
    &= - \left[p^{2\ell-1} \gamma(p)\right]_{-\infty}^\infty + (2\ell-1)\int_{-\infty}^\infty p^{2\ell-2} \gamma(p) ~dp \\
    &= (2\ell-1)c^\gamma_{2\ell-2}.
  \end{align*}
  Thus by induction,
  \[
    c^\gamma_{2\ell} = (2\ell - 1)(2\ell - 3) \cdots c_0 = (2\ell-1)!!.
  \]
  ...
  so that
  \[
    c^\gamma_{2\ell} = \frac{2^{\ell}}{\sqrt{\pi}} \Gamma\left(\ell + \frac12\right) = (2\ell - 1)!!
  \]
\end{myexample}
...
\begin{equation}\label{eq:gaussPythag}
  \gamma_n(x) = \gamma_{n-1}(x - p\omega)\gamma(p)
\end{equation}
...
\]
Thus the $\gamma_n(x)$ is the product of $n$ copies of $\gamma$. The standard Gaussian measure $\gamma^n$ is thus a measure whose cardinal projections are standard Gaussian measures.

% \begin{definition}
% Let $\gamma^n$ be the Borel measure on $\RR^n$ with density $w_n$,
% \[
%   \gamma^n(A) = \int_{A} w_n(x) ~dx.
% \]
% We will call $\gamma^n$ the \emph{standard Gaussian measure} on $\RR^n$. 
% % A more general notion can be defined by a $n \times n$ covariance matrix $\Sigma$, and vector $a \in \RR^n$. If $\Sigma$ is a symmetric positive definite matrix we define the \emph{Gaussian measure} $d\gamma_{\Sigma, a}$, 
% % \begin{align*}
% %   \int_{\RR^n} f(x) d\gamma^n_{\Sigma, a}
% %   &= \frac{{(2\pi)}^{-\frac n2}}{\sqrt{\det\Sigma}}  \int_{\RR^n} f(x) e^{-\frac12\Sigma x, x}} ~dx
% % \end{align*}
% % can be called \emph{Gaussian measures}
% \end{definition}

\begin{myexample}
  Let $f:\RR^n \rightarrow \RR$ be the constant function $f(x) = 1$. The Gaussian moments of $f$ are the multivariate moments of $\gamma_n(x)$, and can be written
  \begin{align*}
    c^\gamma_\alpha 
    &= \int_{\RR^n} x^\alpha \gamma_n(x) ~dx \\
    &= \prod_{i = 1}^n \int_{-\infty}^\infty x_i^{\alpha_i} \gamma(x_i) ~dx \\
    &= \prod_{i = 1}^n c^\gamma_{\alpha_i}
  \end{align*}
  where $\{c^\gamma_k\}_{k \in \NN}$ are the univariate moments of $\gamma(p)$.
  Thus $c^\gamma_\alpha = 0$ if any $\alpha_i$ is odd. Otherwise, if every $\alpha_i$ is even then
  \[
    c^\gamma_\alpha = \prod_{i = 1}^n (\alpha_i - 1)!!
  \]
\end{myexample}
...
We can derive a recurrence relation for $H_k$ with some basic differential calculus. First multiplying both sides by $(-1)^k \gamma(p)$, we get
\[
...
\[
  \gamma^{(k+1)}(p) 
    = {(-1)}^{k} (-p \gamma(p) H_k(p) + \gamma(p)H_k'(p)) 
    = {(-1)}^{k+1} \gamma(p)(pH_k(p) - H_k'(p))
\]
...
\begin{align}\label{eq:HOrth}
  \int_{-\infty}^\infty H_\ell(p) H_k(p) \gamma(p)~dp 
  = 0
\end{align}
...
\begin{align*}
  (-1)^k \left(\left[H_k(p)\gamma^{(k-1)}(p)\right]_{-\infty}^\infty - \int_{-\infty}^\infty H'_\ell(p) \gamma^{(k-1)}(p)~dp\right) \\
  = (-1)^{k-1}\int_{-\infty}^\infty H'_\ell(p) \gamma^{(k-1)}(p)
\end{align*}
where the boundary term $\left[H_k(p)\gamma^{(k-1)}(p)\right]_{-\infty}^\infty$ vanishes because of $\gamma(p)$'s rapid decay at $\pm \infty$. Repeat this process until $H_\ell$ is annihilated.

% \begin{remark}
%   We describe (\ref{eq:HOrth}) in terms of ``orthogonality'' because 
% \end{remark}

The Hermite polynomials form a complete orthogonal basis for $L^2(\gamma)$, the space of function $f : \RR \to \RR$ for which $\int_{-\infty}^\infty f(p)^2 \gamma(p)~dp < \infty$. This is a Hilbert space with inner product $\langle f, g \rangle = \int_{-\infty}^\infty |f(p)g(p)| \gamma(p)~dp$.

\begin{proposition}
   The linear span of the Hermite polynomials $H_0(p), H_1(p), \ldots$ is dense in $L^2(\RR, \gamma)$.
\end{proposition}

% \begin{proof}
%   Note that since there are Hermite polynomials of all degrees $k \geq 0$, their linear span is the set of all polynomials. Thus the proposition is equivalent to polynomials being dense in the weighted $L^2$ space. 
% \end{proof}

% \[
%   (\int_\RR f(p) p^k d\gamma)^2 \leq \int_{\RR} f(p)^2 d\gamma \int_{\RR} p^{2k} d\gamma < \infty
% \]
% \[
%   \inner{f , p^k}^2 \leq \|f\|_2^2 \|p^k\|_2^2
% \]

% \begin{proof}
%   \[
%     f(x) = \sum_{k = 0}^\infty b_k H_k(x), \quad 
%     b_k = \int_{\RR} f(x) H_k(x) \gamma(x) dx
%   \]
%   Assume $f \neq g$ have the same moments $c_k^\gamma$. Let
%   \[
%     g(x) = \sum_{k = 0}^\infty a_k H_k(x), \quad 
%     a_k = \int_{\RR} g(x) H_k(x) \gamma(x) dx
%   \]
%   Then $a_k = b_k$.
%   \[
%     \int_\RR \left(g(x) - \sum_{k = 0}^N a_k H_k(x)\right)^2 d\gamma \to 0
%   \]
%   x
% \end{proof}


In the chapter \ref{chap:GRTpoly} we will make use of a different definition for the Hermite polynomials, as Taylor series coefficients for an exponential generating function.
\begin{lemma}
...
\[
  e^{pt - \frac{t^2}2} 
    ...
    = \frac{\gamma(p - t)}{\gamma(p)}
\]
so that the Taylor coefficients are precisely
\[
  \left[\frac{d^k}{dt^k}\frac{\gamma(p - t)}{\gamma(p)}\right]_{t = 0} 
    = (-1)^k \frac{\gamma^{(k)}(p)}{\gamma(p)} = H_k(p).
\]
\end{proof}
...
\[
  H_\alpha(x) 
    = (-1)^{|\alpha|}\frac{\partial_{\alpha}\gamma_n(x)}{\gamma_n(x)}
\]
where $\partial_\alpha = \frac{\partial^{\alpha_1 + \cdots + \alpha_n}}{\partial x_1^{\alpha_1} \cdots \partial x_n^{\alpha_n}}$ is a partial differential operator. Just like the classical case, this formula can be used to define $H_\alpha$ as multivariate Taylor coefficients for the generating function:
\[
  \phi(x) = e^{\langle x, y\rangle - \frac{\|y\|^2}2} 
    = \frac{\gamma_n(x - y)}{\gamma_n(x)} 
...
\section{The Radon and Gaussian Radon transforms}\label{sec:RTGRT}
...
The Radon transform~\cite{Helg65} (RT) is a thing that I will discuss the history of, with references, in this paragraph. The transform gets its name from Johann Radon, whose first defined the transform in the form below in 1917, although a similar transform was introduced by Paul Funk in 1911. It is interesting to note that the primary application of the RT in medical imaging (CT scans) was not invented for another half century. 
% I can only hope that in 2076 my dissertation will serve as an absorbent coffee coaster for a sleep deprived student.
...
\begin{myexample}
  ...
  \]
  Note that as a bounded domain, we can guarantee $B(r)$ is integrable on all hyperplanes. In fact, the RT of any subset $A \subseteq B(r)$ is bounded by 
  \[
    R_{A} \leq V_{n-1}(r).
  \]
  Also note that $B(r)$ is rotation invariant, hence $R_{B(r)}(\omega, p)$ is independent of $\omega$.
...
Our main addition to previous work will be the use of a modified RT, the Gaussian Radon transform (GRT) \cite{Miha04} \cite{Seng12} \cite{Seng14}. This transform is very similar to the RT, but the inclusion of a Gaussian density $\gamma_{n-1}(x)$ in the integral allows for convergence on a larger class of functions $f$. This includes for example unbounded regions. In a broader context the Gaussian Radon transform also has the advantages of generalizing to infinite dimensional Hilbert spaces~\cite{Seng14} (on which the Lebesgue measure is not defined), as well as having a natural probabilistic interpretation.
...
\[
  GR_f(\omega, p) = 
  \int\mclimits_{\langle x, \omega \rangle = p} f(x) \gamma_{n-1}(x - p\omega) ~dx.
\]
...
\begin{remark}
  ...
  \begin{align*}
    \int\mclimits_{\langle x, \omega \rangle = p} f(x) \gamma_n(x) ~dx
    &= \int\mclimits_{\langle x, \omega \rangle = p} f(x) \gamma_{n-1}(x) ~dx~ \gamma(p),
  \end{align*}
  we can express the GRT of $f$ in terms of the $RT$ of the function $g(x) = f(x)\gamma_n(x)$:
  \begin{equation}
    \label{eq:GRTPythag}
    R_g(\omega, p) = GR_f(\omega, p) \gamma(p), \qquad 
    g(x) := f(x)\gamma_n(x).
  \end{equation}
...
\begin{myexample}
  ...
  \[
    GR_f(\omega, p) = \int_{\RR^{n-1}}f(x(t)) \gamma_{n-1}(t) dt
  \]
...
\begin{myexample}
  ...
  \[
      GR_A(\omega, p) 
      \leq \int\mclimits_{\inner{x, \omega} = p} \gamma_{n-1}(x) ~dx = 1
      % \leq GR_1(\omega, p) 
      % = \int_{\RR^{n-1}} \gamma_{n-1}(t)~dt ~\gamma(p) 
      % = \gamma(p)
  \]
\end{myexample}

% \begin{myexample}
%   Consider the strip $S \subseteq \RR^2$ from example \ref{ex:RTStrip}. While the RT of $S$ was divergent for $\theta = \pi/2$ or $3\pi/2$, the GRT converges. In particular,
%   \[
%     GR_S(\theta, p)
%     = \begin{cases}
...
Now, imagine sweeping a hyperplanar ``slice'' across $\RR^n$. As a function of $p$, $R(\omega, p)$ can be seen as a projection of $f$ onto the linear subspace spanned by $\omega$. Intuitively, since the parallel slices covere the entirety of the space with no overlap, it is not surprising that integrating this projection over $-\infty < p < \infty$ we get the same result as the $n$-fold integral of $f$ over $\RR^n$.
\[
...
\begin{proposition}[Slice Theorem]
  ...
  % If $f \in L^1(\RR^n)$ and $F$ is bounded, then
  ...
  \begin{enumerate}
    \item $f \in L^1(\RR)$ and $F$ is bounded,
    \item $f \in L^1(\RR)$ has compact support and $F$ is locally bounded.
  \end{enumerate}
  % so long as one of the two is finite.
\end{proposition}

\begin{proof}
  Pulling $F(p)$ into the RT integral, we can substitute $p = \inner{x, \omega}$,
  \begin{align*}
    \int_{-\infty}^\infty R_f(\omega, p) F(p) ~dp
    &= \int_{-\infty}^\infty \int_{\inner{x, \omega} = p} f(x) F(\inner{x, \omega}) ~dx ~dp
  \end{align*}
  then it remains to show that the itterated integrals can be combined. This is a relatively straightforward application of Fubini's theorem. Assume for example $\omega = (0, \ldots, 0, 1)$, so that $\inner{x, \omega} = x_n$ for any $x = (x_1, \ldots, x_n) \in \RR^n$. In this simple case Fubini's theorem says for any measurable function $g : \RR^n \to \CC$ 
  \begin{align*}
    \int_{-\infty}^\infty \int_{\RR^{n - 1}} g(x_1, \ldots, x_{n - 1}, p) ~dx_1 \cdots dx_{n - 1} ~dp
    &= \int_{\RR^n} g(x_1, \ldots, x_n) dx
  \end{align*}
  ...
  \begin{enumerate}
    \item Since $F$ is bounded, say $|F| \leq M$ for some $M > 0$. We have
  \[
    \int_{\RR^n} |f(x) F(\inner{x, \omega})| ~dx 
    \leq \int_{\RR^n} |f(x)| M ~dx 
    = M \int_{\RR^n} |f(x)| ~dx < \infty
  \]
  since $f \in L^1(\RR^n)$.
  \item If $\supp(f)$ is compact then so is $\{p : \inner{x, \omega} = p \text{ for some } x \in \supp(f)\}$, since the mapping $x \mapsto \inner{x, \omega}$ is continuous. Thus $F(\inner{x, \omega})$ is bounded on $\supp(f)$ and the rest of the proof is identical to the previous case.
  \end{enumerate}
  
  % We will first prove the special case when $\omega$ is $n$th standard basis vector,
  % \[
  %   \omega = e_n = (0, \ldots, 0, 1) \in \RR^n.
  % \]
  % In this case the proposition is related to Fubini's theorem. Identifying $x \in \RR^n$ with $(x', x_n) \in \RR^{n - 1} \times \RR$, where $x' = (x_1, \ldots, x_{n - 1})$, we note that $\inner{x, e_n} = x_n$, so that can rewrite the RT,
  % \[
  %   R_f(e_n, p) = \int_{\RR^{n - 1}} f(x', p) ~dx'.
  % \]
  % Here it should be noted that as an immediate consequence of Fubini's theorem, the functions $p \mapsto R_f(e_n, p)$ is measurable and finite for almost every $p \in \RR$, and furthermore we get the basic slice theorem
  % \[
  %   \int_{\infty}^\infty R_f(e_n, p)~ dp 
  %   = \int_{\RR} \int_{\RR^{n - 1}} f(x', p) ~ dx' ~ dp 
  %   = \int_{\RR^n} f(x) ~ dx.
  % \]
  % If $F(p)$ is bounded then
  % \[
  %   \int_{-\infty}^\infty R_f(e_n, p) F(p) ~dp
  %   = \int_{\RR} \int_{\RR^{n - 1}} f(x', p) F(x_n) ~dx' ~dp
  %   = \int_{\RR^n} f(x) ~dx
  % \]

  % Let $\omega^\perp$ . Briefly, we may decompose $\RR^n$ as a direct product $\Lambda \otimes \Lambda^\perp$, where $\Lambda$ is the linear subspace given by $\inner{x, \omega} = 0$ and $\Lambda^\perp$ is the subspace spanned by $\omega$. Thus by Fubini's theorem
  % \[
  %   \int_{-\infty}^\infty R_f(\omega, p) F(p) ~dp
  %   = \int_{-\infty}^\infty \int_{x_n = p} f(x', x_n) F(x_n)~dx' ~dp
  % \]
...
If one is interested in inverting the RT then a prerequisite concern is of course: Is the transform injective? The answer clearly depends on what space we draw the function $f$ from. Radon~\cite{Rado17}~\cite{Rado86} provides a set of sufficient regularity conditions such that the RT is invertible. On the other hand counterexamples have been constructed by, for example, Lawrence Zalcman~\cite{Zalc82}, of continuous and nontrivial functions for which the RT is identically zero.

By way of the relation (\ref{eq:GRTPythag}) we can prove an analogous slice theorem for the GRT.\@ Mihai and Sengupta give a more general result for the GRT on real, separable, infinite Hilbert spaces~\cite{Miha04}.

\begin{proposition}[Gaussian Slice Theorem] 
  Let $f:\RR^n \rightarrow \RR$ and $F:\RR \rightarrow \RR$ be measurable functions. If $f \in L^1(\RR^n, \gamma_n)$ and $F$ is bounded then
  \begin{equation}\label{eq:GST}
    \int_{-\infty}^\infty GR_f(\omega, p)F(p) \gamma(p) ~dp
    = \int_{\mathbb{R}^n}f(x) F(\langle x, \omega\rangle) \gamma_n(x) ~dx. 
  \end{equation}
\end{proposition}
...
\begin{remark}
  ...
  \]
  may be used. Of course, whatever conditions on $f$ and $F$ are sufficient for convergence in (\ref{eq:ST}) are then sufficient conditions to be checked of $f(x)\gamma_n(x)$ and $F(p)\gamma(p)$.
\end{remark}
...
\begin{proof}
  ...
  \[
    \int_{-\infty}^\infty R_g(\omega, p) F(p) ~dp 
    = \int_{\RR^n} f(x)F(\langle x, \omega \rangle) \gamma_n(x) ~dx,
  \]
...
\section{The Radon transform of measures}\label{sec:RTmeasure}
...
\begin{remark}
  The slice theorem is analogous to — in fact a special case of — the change of variables formula for the push-forward measure $R^\omega_\mu$:
  \[
...
\section{The Stieltjes transform}\label{sec:stieltjes}

Let $\mu$ be a Borel measure with finite moments $c_k = \int_{-\infty}^\infty p^k d\mu$. 

\begin{definition}
  The function $g : \CC \to \CC$ defined by
  \[
    g(z) = \int_{-\infty}^\infty\frac{d\mu}{1+zp}
  \]
  is called the Stieltjes transform of a Borel measure $\mu$ on $\RR^n$.
\end{definition}
...
\begin{remark}
  The bound above has a helpful geometric interpretation. For starters we note that,
  \[
...
\begin{remark}
  ...
  \]
  For this reason, although $g(z)$ is defined for all non-real $z$, we will only concern ourselves with its values on the upper half plane $\im z > 0$ going forward. As a consequence in chapter \ref{chap:shapereconstruction} we restrict sample points to the upper half plane to avoid redundancy.
\end{remark}


When $z$ is a real number $g(z)$ may not converge. In particular $g(z)$ does not converge when $z = -1/p$ for some $p$ in the support of $\mu$.  Thus if $\mu$ has bounded support $g$ converges in a neighborhood of $0$. On the other hand if $\mu$ has unbounded support $g(0)$ is not defined. However — as we will see — even in the Hamburger transform case, a formal series expansion at $z = 0$ holds valuable information. The results of this section are described in detail in section 5.6 of~\cite{Bake81}.
...
% \begin{remark}
%   An interesting connection — which I don't know enough about to discuss yet — is beginning to be revealed here between moment problems and analytic functions on the upper half plane (in particular, their behavior near the real boundary). This leads us to the notion of interpolation problems.
% \end{remark}

Evidently the asymptotic series expansion $g(z) \simeq \sum_{k=0}^\infty c_k {(-z)}^k$ is generally formal, having zero radius of convergence except in the Markov case. However as we will see in the next section, rational functions constructed from this formal series exist which approximate $g(z)$ on the upper half plane $\im z > 0$.

Finally, we mention a relationship between the Stieltjes transform of an RT and the ``multivariate Stieltjes'' transform:
\begin{definition}
  The \emph{multivariate Stieltjes transform} of a Borel measure $\mu$ on $\RR^n$ is defined by
  \[
    g(y) = \int_{\RR^n} \frac{d\mu(x)}{1 + \langle x, y\rangle} \simeq \sum_{\alpha \in \NN_0^n} c_\alpha(-y)^\alpha
  \]
  when $\mu$ has finite moments $c_\alpha = \int_{\RR^n} x^\alpha d\mu$. The series expansion is generally formal, but again can be understood via the notion of non-tangential convergence.
\end{definition}

\begin{remark}
  Much like the univariate case, for measures $\mu$ supported on a bounded subset of $\RR^n$, the multivariate Stieltjes transform is well defined in a neighborhood of the origin and equal to the power series expansion. 

  If $\mu$ has unbounded support in $\RR^n$ we can not expect $g(y)$ to be defined on $\RR^n$, and are forced to consider the Stieltjes transform as a function of multiple complex variables $g : \CC^n \to \CC$. Here we see singularities in the integrand when $y \in \CC^n$ satisfies
  \[
    \inner{x, y} = -1, \text{ for some } x \in \RR^n
  \]
...
% \end{proposition}
\begin{proposition}
If $f \in L^1(\RR)$ has compact support then taking $F(p) = {(1+zp)}^{-1}$ in the slice theorem~\ref{eq:ST} we have
\[
  \int_{-\infty}^\infty \frac{R_f(\omega, p)}{1 + zp} ~dp = \int_{\RR^n} \frac{f(x)}{1 + z\langle x, \omega \rangle} ~dx.
\]
Similarly if $f \in L^1$ with possibly unbounded support by the Gaussian slice theorem~\ref{eq:GST},
\[
  \int_{-\infty}^\infty \frac{GR_f(\omega, p) \gamma(p)}{1 + zp} ~dp = \int_{\RR^n} \frac{f(x)\gamma_n(x)}{1 + z\langle x, \omega \rangle} ~dx.
\]
\end{proposition}

Note that the first relation makes sense in some neighborhood of $z = 0$, while the second applies only if $\im z \neq 0$.

In chapter \ref{chap:shapereconstruction} we note that the integrals above can be seen as particular restrictions of the multivariate Stieltjes transform. That is, for $y \in \CC^n$ of the form $y = z\omega$ where $\im z > 0$ and $\omega \in S^{n - 1}$,
\[
  \int_{\RR^n} \frac{f(x)}{1 + \inner{x, y}} ~dx = \int_{-\infty}^\infty \frac{R_f(\omega, p)}{1 + zp} ~dp.
\]
Thus we consider the multivariate Stieltjes transform to be defined for $y \in \CC^n$ in this form, as guarenteed by the relation above. An equivalent relation holds for the GRT via the previous proposition.

\section{Pad\'e approximants}\label{sec:pade}
...
Note that the two definitions are equivalent if $b_0 = Q^{L/M}(0) \neq 0$. Hence a sufficient condition for the existence of the Pad\'e approximant by Baker's definition is that $b_0 = Q^{L/M}(0) \neq 0$. 
...
Let $R_M(z)$ be the off-diagonal $[M/M+1]$ approximant to the Hamburger series of $\mu$,
\[
  R_M(z) 
  = \frac{P_M(z)}{Q_M(z)} 
  \approx \sum_{k=0}^\infty c_k {(-z)}^k 
  % \simeq \int_{-\infty}^\infty \frac{f(p)}{1 + zp} ~dp
\]
where $c_k = \int p^k f(p) dp$. 

The following theorem, paraphrased from Baker~\cite{Bake81}, is central to the shape reconstruction method discussed in Chapter 3. 

\begin{definition}
  A family of continuous functions $\mathscr{F}$ is called \emph{normal} if every sequence in $\mathscr{F}$ contains a subsequence which converges \emph{locally uniformly}, that is, uniformly on compact sets.
\end{definition}

\begin{proposition}
  The off-diagonal Pad\'e approximants $R_M(z)$ to a solvable Hamburger series $\sum_{k = 0}^\infty c_k {(-z)}^k$ form a normal family. The limit of a convergent subsequence of approximants must have an integral representation
  \[
    \int_{-\infty}^\infty \frac{d\mu}{1 + zp}
  \]
  where $\mu$ is some Borel measure with moments $c_k$.
\end{proposition}

% As a corollary, if the moments are determinate then all subsequences of the off-diagonal approximants converge locally uniformly to the Hamburger transform of the unique Borel measure $\mu$ with the given moments. Thus the entire sequence converges in the same way. 

\begin{corollary} \label{cr:determinacypade}
  If the moments $c_k$ are determinate, then the off-diagonal approximants to the Hamburger series converge locally uniformly to the Hamburger transform of $\mu$.
\end{corollary}
We discuss sufficient conditions for determinacy in section \ref{sec:momentproblem} and chapter \ref{chap:RTmoments}.

Finally it should be noted that the shape reconstruction method of \cite{Cuyt05} utilized a certain multivariate generalization of the Pad\'e approximants defined in \cite{Cuyt84}. Since a defining feature of these multivariate Pad\'e approximants is that they coincide with Baker's univariate approximants on slices — precisely corresponding to our method of relating the multivariate Stieltjes transform to the univariate Stieltjes transform of projections — we choose to skip this description.
...
%   \]
%   Since this bound is independent of $R_M$, Montel's theorem implies that the off-diagonal Pade approximants form a normal family. It can be shown that the limit of any convergent subsequence has a representation $\int (1+ xz)^{-1}d\sigma(x)$ where $\sigma$ is a solution to the Hamburger moment problem. Since the moment problem is determinate, the sequence $R_M$ must converge to $g(z)$ uniformly on compact sets.
% \end{proof}
...
\section{Symmetric tensors and polynomials} \label{sec:tensors}
In chapter~\ref{chap:GRTpoly} we make some limited use of a tensor notation for multivariable polynomials. For reference we record some basic concepts in this section.
...
\begin{tikzcd}
  V^k \arrow[r, hook] \arrow[dr] & V^{\otimes k} \arrow[d] \\
  & W 
\end{tikzcd}
\end{figure}
