...
\]
which is analytic at $z = 0$ converging to the Stieltjes transform
\[
  % g_{\omega}(z) 
  % = 
  \int_{-\infty}^\infty \frac{R_A(\omega, p)}{1 + pz} ~dp = \int_{A} \frac1{1 + z\inner{x, \omega}}~dx. 
\]
Recall that since $A$ is bounded, it has determinate multivariate moments and projection moments. By corollary~\ref{cr:determinacypade}, $g_{\omega}(z)$ is the uniform limit of the off-diagonal Pad\'e approximants $R_M(z)$. The rate of convergence in this case can be controlled further by results found in~\cite{Bake81}. 

On the other hand suppose $g_\omega(z)$ can be approximated by a cubature formula
\[
  \sum_{i = 0}^I w_i(z) 1_A(x_i)  \approx \int_{A} \frac1{1 + z\inner{x, \omega}}~dx  
\]
