...
\[
  g_{\omega}(z) 
  = 
  \int_{-\infty}^\infty \frac{R_A(\omega, p)}{1 + pz} ~dp = \int_{A} \frac1{1 + z\inner{x, \omega}}~dx. 
\]
...
\]
where $y_0, \ldots, y_J \in \RR^n$ are a set of distinct sample points and $J > I$. At least heuristically, we expect a solution to this system in the form of the values to approximate the values of the characteristic function
\[
...
\[
  c^G_\alpha = \int_{\RR^n} f(x) x^\alpha \gamma_n(x) ~dx.
\]
The Gaussian projection moments for any $\omega \in S^{n-1}$ are given by
\[
  c^G_k(\omega) = \sum_{|\alpha| = k} \binom{k}{\alpha} c^G_\alpha \omega^\alpha.
\]
Now the Stieltjes series
\[
  \sum_{k = 0}^\infty c^G_k(\omega) (-z)^k
\]
is generally formal. Likewise, the Stieltjes transform
\[
  g_\omega(z) = \int_{-\infty}^\infty \frac{GR_A(\omega, p)}{1 + pz} \gamma(p)~dp = \int_A \frac{1}{1 + z\inner{x, \omega}} \gamma_n(x) ~dx
\]
may be undefined on the real line, but is analytic on the upper half plane $\im z > 0$. In this case the Stieltjes series is an asymptotic expansion to the Stieltjes transform in the sense of nontangential convergence,
\[
  g_\omega(z) \simeq \sum_{k = 0}^\infty c^G_k(\omega) (-z)^k.
\]
For more details on the Stieltjes series and transform in the case of Hamburger moments see section \ref{sec:stieltjes}. 

Now we note that $GR_A(\omega, p)$ is a bounded function, so that the Gaussian projection moments are determinate. Thus, as discussed in section \ref{sec:pade}, the $[M-1/M]$ Pad\'e approximants to the Stieltjes series converge locally uniformly on the upper half plane to the Stieltjes transform.

Once again, we consider $g_\omega(z)$ as a restriction of a multivariate Stieltjes transform. For $z \in \RR$ in the upper half plane $\im z > 0$ and $\omega \in S^{n-1}$ we have 
\[
  g_\omega(iz) 
  = \int_A \frac{1}{1 + iz\inner{x, \omega}} \gamma_n(x) ~dx 
\]
Now 
% \[
%   g_\omega(z) 
