...
\section{The GRT of multivariate Hermite polynomials}


Recall that for $\alpha \in \NN_0^n$ the polynomials $H_\alpha(x)$ can be defined by the generating function
\[
...
\begin{proof}
  ...
  \begin{align*}
    GR_{\phi}(\omega, p) 
      &= \int\mclimits_{\langle x, \omega \rangle = p} \phi(x) \gamma_{n-1}(x - p\omega)~dx.
  ...
  \begin{equation} \label{eq:GRTPhiExpansion1}
    \begin{split}
      GR_{\phi}(\omega, p)
        &= \sum_{\alpha \in \NN_0^n} \frac1{\alpha!} y^\alpha \int\mclimits_{\langle x, \omega\rangle = p} H_\alpha(x) \gamma_{n-1}(x - p\omega)~dx
  ...
  \begin{equation}\label{eq:GRTPhiExpansion21}
    \begin{split}
      GR_{\phi}(\omega, p) 
        &= \int\mclimits_{\langle x, \omega\rangle = 0} \phi(x + p\omega) \gamma_{n-1}(x) ~dx 
...
\begin{remark}
  The proof above was written by Dr. Sengupta. Recall that the Hermite polynomials $H_\alpha$ form a complete orthogonal basis for $L^2(\RR^n, \gamma_n)$. In this sense the formula (\ref{eq:GRH}) completely defines the Gaussian Radon transform on this space. Indeed, it can be shown that any function $f \in L^2(\RR^n, \gamma_n)$ has an expansion
  \[
    f(x) = \sum_{\alpha \in \NN_0^n} a^{(f)}_\alpha H_\alpha(x)
  \]
  where
  \[
    a_\alpha^{(f)} = \int_{\RR^n} f(x)H_\alpha(x) \gamma_n(x) ~dx.
  \]
  If this expansion converges nicely on $\Lambda$ then
  \begin{align*}
    GR_f(\Lambda) 
    &= \int_\Lambda f(x) \gamma_n(x-p_0)~dx 
    \\ 
    &= \int_\Lambda \sum_{\alpha \in \NN_0^n} a_\alpha^{(f)} H_\alpha(x) \gamma_n(x-p_0) ~dx
    \\
    &= \sum_{\alpha \in \NN_0^n} a_\alpha^{(f)} \int_\Lambda H_\alpha(x) \gamma_n(x-p_0) ~dx
    \\
  ...
  \end{align*}
  For details on the convergence of these expansions see~\cite{DuXu01} or~\cite{Than93}. 
\end{remark}
...
\]
By this embedding can define the Euclidean measure $dx$ on $\Lambda$ as the pushforward of the Lebesgue measure on $\RR^n$, and the standard Gaussian weight on $\Lambda$ as $w_d(t)$. Note the $x(t)$ is defined in such a way that $x(0) = v$ which is essential when defining the Gaussian measure on $\Lambda$.

Other times we prefer not to invoke a basis-dependent isometry, and while $x(t)$ clearly depends on the choice of $u^{1}, \ldots, u^{(d)}$, we should note that both the Euclidean and Gaussian measures on $\Lambda$ are invariant under orthogonal transformations about $v$ and thus independent of basis.
...
\begin{definition}
  ...
  \]
  where $d$ is the dimension of the linear subspace $\Lambda_0$. In both definitions $dx$ denotes the $d$-dimensional Lebesgue measure on $\Lambda$.
\end{definition}
\begin{remark}
  Explicitly, if $x(t): \RR^d \rightarrow \Lambda$ is a Euclidean isometry then 
  \[
    R_f(\Lambda) = \int_{\RR^d} f(x(t))~dt.
  \]
  If furthermore $x(0) = p\omega$ then
  \[
    GR_f(\Lambda) = \int_{\RR^d} f(x(t))\gamma(t)~dt.
  \]
  However keep in mind that these definitions are independent of the particular isometry.
\end{remark}

% \begin{remark}
%   More generally one the $d$-plane GRT can be seen as a 
%   \[
%     \frac{(2\pi)^{-\frac{n}2}}{\sqrt{\det(\Sigma)}} \int_{\RR^n} e^{-\frac12 \inner{\Sigma^{-1}x, x}} ~dx
%   \]
% \end{remark}

In order to generalize the formula (\ref{eq:GRH}) we begin with the integral of $H_\alpha$ over linear subspaces. Recall that the GRT of a function $f$ over a linear subspace $\Lambda_0$ can be written
\begin{equation} \label{eq:GRRot}
  GR_f(\Lambda_0) = \int\mclimits_{\RR^d \times {\{0\}}^{n-d}} f(Rx) \gamma_d(x) ~dx
\end{equation}
...
\begin{proof}
  ...
  \]
  Using the tensor power notation from section \ref{sec:tensors}, we can write
  \[
  ...
  \]
  follows from the definition of the inner product on ${(\RR^n)}^{\otimes k}$ and further notation from section \ref{sec:tensors}.
\end{proof}
\begin{remark}
...
\begin{proposition}
  ...
  \]
  where $c^G_\beta = \int_{\RR^d} x^\beta \gamma_d(x) ~dx$ are the Gaussian moments on $\RR^d$.
\end{proposition}

\begin{proof}
  We take (\ref{eq:GRRot}),
  \[
    GR_f(\Lambda_0) = \int\mclimits_{\RR^d \times {\{0\}}^{n-d}} {(Rx)}^\alpha \gamma_d(x) ~dx,
  \]
  and expand by (\ref{eq:monRot}),
  \begin{align*}
    \int\mclimits_{\RR^d \times {\{0\}}^{n-d}} (Rx)^\alpha \gamma_d(x) ~dx
    &= \sum_{\substack{\beta \in \NN_0^n \\ |\beta| = k}} {c(R)}^\alpha_\beta \int_{\RR^d \times {\{0\}}^{n-d}} x^\beta \gamma_d(x) ~dx
  \end{align*}
  as needed.
\end{proof}
